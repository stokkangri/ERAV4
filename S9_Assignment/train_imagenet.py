"""
Main training script for ResNet-50 on ImageNet
"""
...
class ImageNetTrainer:
    """
    Complete training pipeline for ResNet-50 on ImageNet
    """
    ...
    def setup_data(self):
        ...
        # Create data loaders with subset support
        self.train_loader, self.val_loader, self.dataset_stats = create_imagenet_loaders(
            data_dir=self.config['data_dir'],
            batch_size=self.config['batch_size'],
            num_workers=self.config['num_workers'],
            subset_percent=self.config.get('subset_percent', None),
            tiny_imagenet=self.config.get('tiny_imagenet', False),
            augment_train=self.config['augment_train']
        )
        
        print(f"Dataset: {'Tiny ImageNet' if self.config.get('tiny_imagenet', False) else 'ImageNet'}")
        print(f"Train samples: {self.dataset_stats['num_train_samples']:,}")
        print(f"Val samples: {self.dataset_stats['num_val_samples']:,}")
    ...
    def setup_model(self):
        ...
        self.model = resnet50(
            ...
            replace_maxpool_with_conv=self.config.get('replace_maxpool_with_conv', True)
        )
        
        if self.config.get('replace_maxpool_with_conv', True):
            print("Using Conv layer instead of MaxPool")
        else:
            print("Using standard MaxPool layer")
    ...
    def setup_training(self):
        ...
        if self.config['scheduler'] == 'onecycle':
            self.scheduler = OneCycleLR(
                ...
                final_div_factor=self.config.get('final_div_factor', 10000.0)
            )
    ...
    def find_lr(self, start_lr: float = 1e-7, end_lr: float = 10, num_iter: int = 200):
        """
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.config['epochs']):
            ...
            print(f"  Val Top-5 Acc: {test_acc_top5:.2f}%")
        ...
        print(f"Best Validation Top-5 Accuracy: {max(self.history['test_acc_top5']):.2f}%")
        print("="*50)
    ...
    def plot_progress(self, save_path: Path = None):
        ...
        axes[0, 0].plot(epochs, self.history['test_loss'], 'r-', label='Val')
        ...
        axes[0, 1].plot(epochs, self.history['test_acc'], 'r-', label='Val')
        ...
        axes[0, 2].set_title('Top-5 Accuracy (Val)')
        ...
        best_test_acc_top5 = max(self.history['test_acc_top5']) if self.history['test_acc_top5'] else 0
        
        summary_text = f"""Training Summary:
        
Best Train Acc: {best_train_acc:.2f}%
Best Val Acc: {best_test_acc:.2f}%
Best Val Top-5 Acc: {best_test_acc_top5:.2f}%

Final Train Acc: {self.history['train_acc'][-1]:.2f}%
Final Val Acc: {self.history['test_acc'][-1]:.2f}%
Final Val Top-5 Acc: {self.history['test_acc_top5'][-1]:.2f}%
...
Final LR: {self.history['lr'][-1]:.2e}"""
        ...
        dataset_name = 'Tiny ImageNet' if self.config.get('tiny_imagenet', False) else 'ImageNet'
        plt.suptitle(f'ResNet-50 {dataset_name} Training Progress', fontsize=16)
        plt.tight_layout()
...
def get_default_config():
    ...
    return {
        ...
        'num_workers': 8,
        'augment_train': True,
        'subset_percent': None,  # Use full dataset by default
        'tiny_imagenet': False,  # Use full ImageNet by default
        ...
        'pretrained': False,
        'multi_gpu': True,
        'replace_maxpool_with_conv': True,  # Use Conv instead of MaxPool by default
...
def main():
    ...
    parser.add_argument('--subset-percent', type=float, default=None,
                       help='Use only this percentage of data (0-1)')
    parser.add_argument('--tiny-imagenet', action='store_true',
                       help='Use Tiny ImageNet instead of full ImageNet')
    ...
    parser.add_argument('--no-replace-maxpool-with-conv', action='store_true',
                       help='Use standard MaxPool instead of strided Conv')
    ...
    # Directory arguments
    parser.add_argument('--checkpoint-dir', type=str, default='./checkpoints/imagenet',
                       help='Checkpoint directory')
    parser.add_argument('--log-dir', type=str, default='./logs/imagenet',
                       help='Log directory')
    parser.add_argument('--plot-dir', type=str, default='./plots/imagenet',
                       help='Plot directory')
    ...
    config['subset_percent'] = args.subset_percent
    config['tiny_imagenet'] = args.tiny_imagenet
    config['pretrained'] = args.pretrained
    config['multi_gpu'] = not args.no_multi_gpu
    config['replace_maxpool_with_conv'] = not args.no_replace_maxpool_with_conv
